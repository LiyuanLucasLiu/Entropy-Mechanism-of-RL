...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            actor_lr_scheduler = get_constant_schedule_with_warmup(optimizer=actor_optimizer, num_warmup_steps=0)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_entropy_for_every_source(self, data: DataProto):
        ...
        output_entropy_val, output_entropy_loss_val = self.actor.compute_entropy_for_every_source(data=data)

        log_gpu_memory_usage('After compute entropy', logger=logger)
        
        output = DataProto.from_dict(tensors={'output_entropy_val': output_entropy_val,
                                              'output_entropy_loss_val': output_entropy_loss_val})
        output = output.to('cpu')
